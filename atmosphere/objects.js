...
export function buildFurnitureCatalog(house, tab) {
  const out = {};
  const furn = (house?.furniture || []).filter(
    (f) => (f.room || f.Room || "") === tab || (tab === "common" && (f.room === "common" || f.Room === "Common"))
  );
  ...
  list.forEach((f, i) => {
    ...
    const id = f.id || f.Id || f.furnitureId || `${inferType(displayName)}_${String(i + 1).padStart(2, "0")}`;
